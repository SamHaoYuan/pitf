...
torch.manual_seed(1)
np.random.seed(1)
from NeuralPITF import TransPITF, SinglePITF_Loss, DataSet
...
def train(data, test):
    ...
    model = TransPITF(num_user, num_item, num_tag, dim, init_st).cuda()
    torch.save(model.state_dict(), 'trans_initial_params')
    # 对每个正样本进行负采样
    ...
    # torch.save(model, "trans_net.pkl")
    # torch.save(model.state_dict(), "trans_net_params.pkl")
